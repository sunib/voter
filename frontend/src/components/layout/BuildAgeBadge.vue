...
const frontendBuildAge = computed(() => getBuildAgeLabel(buildInfo.buildTimestamp, now.value))
...
<template>
  <div class="pointer-events-none fixed right-3 top-3 z-40">
      ...
      >
        <div class="space-y-2">
          ...
          <div v-if="backendBuildInfo">
            ...
            <div class="mt-0.5 text-black/48">
              {{ backendBuildInfo.commitWithDirty }} • {{ backendBuildInfo.buildDate }}
            </div>
          </div>
          ...
          <div v-else class="text-black/48">
