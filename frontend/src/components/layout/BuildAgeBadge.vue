...
<template>
  <div class="pointer-events-auto group relative">
    <button
      type="button"
      class="inline-flex rounded-full border border-[rgb(var(--line))]/8 bg-[rgb(var(--card))]/82 px-2 py-1 text-[10px] font-medium tracking-[0.08em] text-[rgb(var(--muted))]/60 shadow-[0_6px_18px_rgba(17,12,8,0.06)] backdrop-blur transition-colors hover:border-[rgb(var(--line))]/15 hover:bg-[rgb(var(--card))]/94 hover:text-[rgb(var(--muted))] focus:border-[rgb(var(--line))]/15 focus:bg-[rgb(var(--card))]/94 focus:text-[rgb(var(--muted))] focus:outline-none"
    ...
    >
      <div class="space-y-2">
        ...
        <div v-else-if="backendLoading" class="text-[rgb(var(--muted))]">
          backend-build loading…
        </div>

        <div v-else class="text-[rgb(var(--muted))]">backend-build unavailable</div>
      </div>
    </div>
  </div>
</template>
